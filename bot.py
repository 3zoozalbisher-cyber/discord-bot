...
MEMBER_COUNT_CHANNEL_ID = 1461682139308363779   # PUT REAL CHANNEL ID OR LEAVE 0
VOICE_COUNT_CHANNEL_ID = 1461682183243563164    # PUT REAL CHANNEL ID OR LEAVE 0

LEVEL_ROLES = {
    5: 0,    # level: role_id (replace 0 with real role IDs)
    10: 0,
    20: 0
}
...
async def on_message(message):
    ...
    # Anti-link for new users (24h)
    if re.search(r"https?://", message.content):
        joined = message.author.joined_at
        if joined and (time.time() - joined.timestamp()) < 86400:
            await message.delete()
            return
